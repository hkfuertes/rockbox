...
static jmethodID android_enable_mtp_method = NULL;

int android_enable_mtp(int mtp_enable)
{
    ...
    if (android_enable_mtp_method == NULL) {
        android_enable_mtp_method = (*env_ptr)->GetMethodID(env_ptr, RockboxService_class,
                                                     "enableMTP", "(I)I");
        if (android_enable_mtp_method == NULL) {
    ...
    jint result = (*env_ptr)->CallIntMethod(env_ptr, RockboxService_instance, 
                                 android_enable_mtp_method, (jint)mtp_enable);
